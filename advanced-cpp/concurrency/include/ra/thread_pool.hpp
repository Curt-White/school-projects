...
namespace ra::concurrency {
	...
	class thread_pool {
		private:
			...
			std::condition_variable new_jobs_; // when new jobs are added then signal
			std::condition_variable can_push;  // needed to make is_empty threadsafe
			std::condition_variable all_done_; // signal when thread exits during shutdown

			std::function<void()> make_thread_handler(int id);
		public:
			...
			size_type size() const { return num_threads_; }
