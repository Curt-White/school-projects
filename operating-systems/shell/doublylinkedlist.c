...
void swap(struct node* ref1, struct node* ref2) {
    ...
    // If the refs are directly beside eachother
    if (ref1 -> next == ref2 || ref2 -> next == ref1) {
        // flip if side by side so first ref always first in list
        if (ref2 -> next == ref1) {
        ...
        // ensure node is not at front or back of list, adj nodes might be NULL
        if (ref1 -> prev != NULL) ref1 -> prev -> next = ref2;
        if (ref2 -> next != NULL) ref2 -> next -> prev = ref1;

        // Swap refs prev and next nodes
        ref2 -> prev = ref1 -> prev;
        ref1 -> next = ref2 -> next;
        ref2 -> next = ref1;
        ref1 -> prev = ref2;
        ...
        // ensure node is not at front or back of list, adj nodes might be NULL
        if (ref1 -> prev != NULL) ref1 -> prev -> next = ref2;
        if (ref1 -> next != NULL) ref1 -> next -> prev = ref2;
        if (ref2 -> prev != NULL) ref2 -> prev -> next = ref1;
        if (ref2 -> next != NULL) ref2 -> next -> prev = ref1;

        struct node* temp_prev = ref2 -> prev;
        ref2 -> prev = ref1 -> prev;
        ref1 -> prev = temp_prev;

        struct node* temp_next = ref2 -> next;
        ref2 -> next = ref1 -> next;
        ref1 -> next = temp_next;
    }
}
...
struct node* concat(struct node* ref1, struct node* ref2) {
    // find last node in ref1
    struct node* last = end(ref1);

    // connect the last node to the start of the next list
    last ->next = ref2;
    ref2 -> prev = last;
...
void minmax(struct node* ref, int* min, int* max) {
    ...
    *max = ref -> data;
    ...
    for (struct node* n = begin(ref); n != NULL; n = next(n)) {
...
bool includes(struct node* ref1, struct node* ref2) {
    // Iterate through possible subset list
    for (struct node* n = begin(ref2); n != NULL; n = next(n)) {
        bool found = false;

        // Iterate the linked list
        for (struct node* e = begin(ref1); e != NULL; e = next(e)) {
            if (e -> data == n -> data) {
                found = true;
            }
        }
...
int main() {
    ...
    printf("Swap again 2 and 14: ");
    swap(ffind(begin(list), 2), ffind(begin(list), 14));
    print(list);

    printf("Swap again 266 and 144: ");
    swap(ffind(begin(list), 266), ffind(begin(list), 144));
    print(list);
    ...
    printf("Find Maximum and Minimum\n");
    int min = 0;
    int max = 0;
    minmax(list, &min, &max);
    printf("Min: %i, Max: %i\n", min, max);
